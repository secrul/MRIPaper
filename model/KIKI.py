...
class KCNN(nn.Module):
    def __init__(in_c = 1, out_c = 1, chans = 32):
        super().__init__()
        self.in_c = in_c
        self.out_c = out_c
        self.chans = chans

        self.convr = nn.Conv2d(1, self.chans, 3, 1, 1, bias = True)
        self.convi = nn.Conv2d(1, self.chans, 3, 1, 1, bias = True)
...
class KIKI(nn.Module):
    """
    KIKI-net: cross-domain convolutional neural networks for
