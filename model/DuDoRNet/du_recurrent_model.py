...
class RecurrentModel(nn.Module):
    ...
    def forward(self,x, k, mask):
        ...
        for i in range(1, self.n_recurrent + 1):
            ...
            #源代码里面是重建结束求loss，但是我认为应该dc后求loss吧;这个版本是先dc后求loss
            tmp = self.net_G_K(x_K)  # output recon kspace
            ans_K[i-1], _ = self.dcs_K[i - 1](tmp, k, mask)  # output data consistency images
