...
import sys
sys.path.append('/home/data/ljh/fastmri/MRI/model/TDC')
...
class convBlock(nn.Module):
    def __init__(self, ioChannel = 2, iConvNum = 10, f=32, dilationLayer = False):
...
if __name__ == '__main__':
    ...
    gt_k = torch.rand(2,256,256,2)
    mask = torch.rand(2,1,256,1)
    model = CN_Dense(2,c=10,dilate=True, useOri = True, transition=0.5, trick = 2)
    num_params = sum(param.nelement() for param in model.parameters())
    print(num_params / 1e6)
    out = model(x, gt_k, mask)
    print(out.shape)