...
"""
相比于V2，改动之处是cal_sa_mask的输入，v2输入的是channel为1，本版本v4输入的是x_HF
...
class FGNetV2(nn.Module):
    def __init__(
        ...
        self.sig = nn.Sigmoid()
        self.sa_mask_conv = nn.Conv2d(2, 1, 3, padding=1)
    ...
    def forward(self, x, k, mask):
        ...
        x0 = self.convf(x0)
        x_HF_k = fft2c(out1)
        x_HF_s_i = self.cal_sa_mask(x_HF)
        x_HF_s_k = self.cal_sa_mask(x_HF_k)
        
        x1 = x0 + self.b1(x0,x_HF_s_i, x_HF_s_k)
        x2 = x1 + self.b2(x1,x_HF_s_i, x_HF_s_k)
        x3 = x2 + self.b3(x2,x_HF_s_i, x_HF_s_k)
        x4 = x3 + self.b4(x3,x_HF_s_i, x_HF_s_k)
        output = self.last(x4)
