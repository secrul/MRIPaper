...
class RCAB(nn.Module):
    def __init__(
        ...
        self.flag = flag
        self.conv1 = ComplexConv(num_feat, num_feat, kernel_size=kernel_size, padding=1, norm=False)
        self.conv2 = ComplexConv(num_feat, num_feat, kernel_size=kernel_size, padding=1, norm=False, act=None)
        self.conv1_k = ComplexConv(num_feat, num_feat, kernel_size=1, padding=0, norm=False)
        self.conv2_k = ComplexConv(num_feat, num_feat, kernel_size=1, padding=0, norm=False, act=None)
        self.fuse = ComplexConv(num_feat * 3, num_feat, kernel_size=1, padding=0, norm=False, act=None)
        modules_body = []
...
class FGNetV2(nn.Module):
    def __init__(
        ...
        self.feature_ext = nn.Sequential(
            ComplexConv(in_c * 2, num_feat // 2, kernel_size = 3, stride=1, padding = 1, norm=False),
            ComplexConv(num_feat // 2, num_feat, kernel_size = 3, stride=1, padding = 1, norm=False)
        )

        self.last = nn.Sequential(
            ComplexConv(num_feat, num_feat // 2, kernel_size = 3, stride=1, padding = 1, norm=False),
            #1卡是act为True，3卡对比为False
            ComplexConv(num_feat // 2, 1, kernel_size = 3, stride=1, padding = 1, norm=False)
        )
        ...
        self.HF = nn.Sequential(
            ComplexConv(in_c, num_feat // 2, kernel_size = 3, stride=1, padding = 1, norm=False),
            ComplexConv(num_feat // 2, num_feat, kernel_size = 3, stride=1, padding = 1, norm=False),
            RCAB(flag='HF'),
            RCAB(flag='HF')
        )
        ...
        self.convf = ComplexConv(num_feat * 2, num_feat, kernel_size = 3, stride=1, padding = 1, norm=False)
        self.sig = nn.Sigmoid()
