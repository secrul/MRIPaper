...
from knee_single_dc import dataset_fastMRI
import torch
...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,criterion, val_dataloader, epochs, log_name = '/home/data/ljh/fastmri/MRI/train_log',
                    checkpoint_name = '/home/data/ljh/fastmri/MRI/checkpoints'):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            #data:mask_kspace, kspace_norm,  mask_image_2c, gt_image, mask, file_name, slice_num, mean, std, attributes['max']
            step += 1
            gt_kspace = data[1]
            mask = data[4]
            ...
            # t_one = torch.ones_like(mask)
            inv_mask = (1 - mask).float().cuda(non_blocking=True)
            mask = mask.float().cuda(non_blocking=True)
            
            out = model(mask_image, gt_kspace, mask, inv_mask).permute(0, 2, 3, 1)
            out = complex_abs(out)
...
if __name__ == '__main__':
    ...
    dataset_model_lossfunc = 'kneesingledc_unetDC_image_2c_l1'
    
    model = Unet_dc(2, 2, 32, 4, 'in').to(device)
