...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,criterion, val_dataloader, epochs, log_name = '../train_log2',
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            ...
            pre_out = pre_out.float().cuda(non_blocking=True) / gt_image.max()
...
if __name__ == '__main__':
    ...
    print("config")
    print("""
        模型：主loss和辅助loss都包含image和k空间两个l1loss，gamma指定为0.1
        过完备,通道数相同，计算量扩大四倍
    ...
    test_dataset = dataset_Brain(vla_root,premodel='dcnn', ty='val', acc=4, seed=seed)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, shuffle=False, batch_size=1,num_workers=4,pin_memory=True)
    train_dataset = dataset_Brain(train_root,premodel='dcnn', ty='train', acc=4, seed=seed)
    ...
    # dataset_model_lossfunc = 'Braindc_fgnetv2_dcnn_image_2c_l1x4_gamma01'
    dataset_model_lossfunc = 'Braindc_fgnetv2_dcnn_image_2c_l1x4_oneloss'
    # dataset_model_lossfunc = 'Braindc_fgnetoc_dcnn_image_2c_l1x4_oneloss'
