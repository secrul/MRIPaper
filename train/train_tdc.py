...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,criterion, val_dataloader, epochs, log_name = '../train_log',
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            ...
            #[24, 2, 320, 320]  [24, 320, 320, 2]     [24, 1, 320, 1]
            # assert 1 > 5
...
if __name__ == '__main__':
    ...
    test_dataset = dataset_Brain(vla_root, seed = seed, acc=8)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, shuffle=False, batch_size=1,num_workers=4,pin_memory=True)
    train_dataset = dataset_Brain(train_root, seed=seed, acc=8)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, shuffle=False, batch_size=batchsize,num_workers=8,pin_memory=True)
    print(len(test_dataloader))


    criterion = torch.nn.L1Loss()
    dataset_model_lossfunc = 'Brain_TDC_image_2c_l1x8'
    
    model = CN_Dense(2,c=10,dilate=True, useOri = True, transition=0.5, trick = 2).to(device)

    if continue_train:
        # checkpoint_path = '/home/westdata/ljh/checkpoints/knee_single_dc_ffa_perceptual/epoch_{}.pth'.format(start_epoch-1)
        checkpoint_path = '/home/data/ljh/fastmri/MRI/checkpoints/kneesingledc_DCNN_image_2c_l1x8/epoch_37.pth'
