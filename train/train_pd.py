...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,criterion, val_dataloader, epochs, log_name = '../train_log',
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            ...
            mask_kspace = mask_kspace.float().cuda(non_blocking=True)
            mask = mask.float().cuda(non_blocking=True)
            
            out = model(mask_kspace,  mask_image,  mask).permute(0, 2, 3, 1)
            out = complex_abs(out)
...
def val(dataloader, model, epoch,writer):
    ...
    for i,data in enumerate(dataloader): # 每个batch更新一次参数
        ...
        # plt.savefig('/home/data/ljh/Brain_MRI/Brain_MRI/imgs/pdx4/{}_{}.jpg'.format(str(file_name), str(slice_num)))
...
if __name__ == '__main__':
    ...
    if continue_train:
        ...
        checkpoint_path = '/home/data/ljh/Brain_MRI/Brain_MRI/checkpoints/Braindc_PD_image_2c_l1x4/epoch_160.pth'
