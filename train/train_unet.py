...
from fftc import fft2c_old as fftc
...
def val(dataloader, model, epoch, writer=None):
    total_loss = []
    total_psnr = []
    total_ssim = []
    slic = 0
    ...
    for i,data in enumerate(dataloader): # 每个batch更新一次参数
        ...
        out = model(mask_image.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
...
if __name__ == '__main__':
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, shuffle=False, pin_memory=True,batch_size=1,num_workers=4)
    train_dataset = dataset_Brain(train_root, acc=8, seed =seed)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, shuffle=True,pin_memory=True, batch_size=batchsize,num_workers=8)
    print(len(test_dataloader))

    
    criterion = torch.nn.L1Loss()
    dataset_model_lossfunc = 'Braindc_unet_image_2c_l1x8'
    model = Unet(2, 2, 32, 4, 'in').to(device)
    ...
    if continue_train:
        ...
        optimizer.load_state_dict(torch.load('/home/westdata/ljh/checkpoints/knee_single_dc_unet_dc_l1/optimizer.pth'))
    num_params = sum(param.nelement() for param in model.parameters())
    print(num_params / 1e6)
    optimizer = torch.optim.Adam(model.parameters(), lr=learnrate)#, betas = (0.9, 0.999), weight_decay = 1e-7
