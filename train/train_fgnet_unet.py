...
sys.path.append('/home/westdata/ljh/projects/Brain_MRI/Brain_MRI')
...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,criterion1,criterion2,criterionssim, val_dataloader, epochs, log_name = '../train_log2',
                    checkpoint_name = '../checkpoints2'):
    ...
    # gamma = torch.tensor(0.5, requires_grad = True).cuda()
    gamma = 0.01
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            ...
            pre_out = pre_out.float().cuda(non_blocking=True) / std
            mask = mask.float().cuda(non_blocking=True)
            # print(pre_out.shape)
            # assert 1 >8
            gt_image = gt_image.unsqueeze(1)
            pre_out = pre_out.unsqueeze(1)
            ...
            out_1c = complex_abs(out) 
            out1_1c = complex_abs(out1) 
            error_1c = complex_abs(error)
            gt_image_1c = complex_abs(gt_image)
            # print(error.shape)
            # print(out.shape, gt_image.shape,out1.shape, error.shape)
            error_1c = gauss_blur(error_1c)
            # gamma = 0.4 - epoch * 0.02
            # gamma = max(gamma, 0)
            # loss1 = criterion(out, gt_image) + criterion(out_k, gt_kspace)
            # loss1 = criterion1(out, gt_image)
            # loss2 = criterion1(out1, error)
            loss1 = criterion1(out, gt_image) + 0.002 * (1-criterionssim(out_1c, gt_image_1c))
            loss2 = criterion1(out1_1c, error_1c) + 0.002 * (1-criterionssim(out1_1c, error_1c))
...
if __name__ == '__main__':
    ...
    #主loss和辅助loss都包含image和k空间两个l1loss，gamma指定为0.1
    print("config")
    print("""
        模型：unet gamma_0.01, 辅助loss是幅度值加gauss，主loss是只有图像，通道1c
    ...
    criterionssim = SSIM().cuda()
    # dataset_model_lossfunc = 'Braindc_fgnetv2_dcnn_image_2c_l1x4_gamma001_v2'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma001_v2'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma001_v2_guass_oneImgLoss_1c'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma001_v2_guass_oneImgLoss_1c_ssim0.1'
    dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma001_v2_guass_oneImgLoss_2c_ssim0.1'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma_auto'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma_auto_05'
    # dataset_model_lossfunc = 'Braindc_fgnetv2_unet_image_2c_l1x4_gamma_linearMinus'
    # dataset_model_lossfunc = 'Braindc_fgnetoc_unet_image_2c_l1x4_oneloss_flopsx4'
    # dataset_model_lossfunc = 'Braindc_fgnetoc_unet_image_2c_l1x4_oneloss'
    ...
    if continue_train:
        ...
        checkpoint_path = '/home/data/ljh/Brain_MRI/Brain_MRI/checkpoints2/Braindc_fgnetv2_unet_image_2c_l1x4_gamma_linearMinus/epoch_5.pth'
    ...
    train(model, train_dataloader, optimizer, scheduler, dataset_model_lossfunc, criterion1,criterion2,criterionssim, test_dataloader,epoch)
