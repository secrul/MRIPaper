...
def train(model, train_dataloader, optimizer,lr_scheduler, dataset_model_lossfunc,layer_num, val_dataloader, epochs, log_name = '../train_log',
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i,data in enumerate(train_dataloader): # 每个batch更新一次参数
            ...
            [x_output, loss_layers_sym] = model(mask_image, mask)
            x_output = complex_abs(x_output.permute(0, 2, 3, 1))
            gt_image = complex_abs(gt_image)

            loss_discrepancy = torch.mean(torch.pow(x_output - gt_image, 2))
            loss_constraint = torch.mean(torch.pow(loss_layers_sym[0], 2))
            for k in range(layer_num-1):
                loss_constraint += torch.mean(torch.pow(loss_layers_sym[k+1], 2))

            gamma = torch.tensor([0.01], requires_grad=True).to('cuda')
            ...
            print("{}/{},train_loss:{:.6}".format(i, (dt_size - 1) // batch_size + 1, loss.item()))
...
def val(dataloader, model, epoch, writer):
    ...
    for i,data in enumerate(dataloader): # 每个batch更新一次参数
        ...
        out = complex_abs(x_output.permute(0, 2, 3, 1)) * std
...
if __name__ == '__main__':
    ...
    dataset_model_lossfunc = 'Brain_ISTA_image_2c_l1_x8'
    
    model = ISTANetplus(layer_num).to(device)

    if continue_train:
        # checkpoint_path = '/home/westdata/ljh/checkpoints/knee_single_dc_ffa_perceptual/epoch_{}.pth'.format(start_epoch-1)
        checkpoint_path = '/home/data/ljh/fastmri/MRI/checkpoints/kneesingledc_ISTA_image_2c_l1_x8/epoch_5.pth'
    ...
    train(model, train_dataloader, optimizer, scheduler, dataset_model_lossfunc, layer_num, test_dataloader,epoch)
